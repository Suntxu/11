...
class Multop extends Backend
{
    ...
    public function index(){
        ...
        if ($this->request->isAjax()){
            ...
            list($tit,$api_id) = $this->searchTop();
    ...
    public function multire(){
        ...
        if($this->request->isAjax()){
            ...
            ['key' => $key,'status' => $status,'kid' => $kid,'tit' => $data['tit']]
            $data = [];
            $msg = '';
            //检测 
            foreach($ids as $k => $v){
                $res = $this->checkDomainStatus(['id' => $v,'flag' => $param['flag'] ]);
                if(is_array($res)){
                    $data[] = $res;    
                }else{
                    $msg .= $res;
            ...
            // //修改
            array_map(function($v) use ($param) {
                if($param['flag'] == 2){
            ...
            $tits = array_column($data,'tit');
            
            if(empty($tits)){
                $this->error('该批域名不存在任务队列中,请确认！');
            }
            $tis = array_diff($tdomain,$tits);
            
            if($tis){
                $msg = ',以下域名不存在任务队列中,已为您过滤:'.implode(',',$tis);
            }
    ...
    public function searchTop(){
        ...
        $where = json_decode($this->request->param('filter'),true);
        $tit = [];
        $api_id = 0;
    ...
    public function modi(){
        ...
        if($this->request->isAjax()){
            ...
            // var_dump($status);var_dump($key);die;
            if($param['flag'] == 2 ){
    ...
    private function checkDomainStatus($param){
        ...
        if($param['flag'] == 1){ //提交
            ...
            if($opk != 3 || $ofied){
                return $data['tit'].'状态错误,已过滤！';
                // $this->error('域名处理成功状态错误或该域名已经被处理:'.$data['tit']);
            }
            if($data['del_time'] < time()){
            ...
            if(empty($opk) && empty($ofied)){ // 直接处理失败
                ...
                return $data['tit'].'状态错误,已过滤！';
                // $this->error('域名处理失败状态错误或该域名已经被处理:'.$data['tit']);
            }
        }
        return ['key' => $key,'status' => $status,'kid' => $kid,'tit' => $data['tit']];
