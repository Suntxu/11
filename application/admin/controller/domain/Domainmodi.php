...
class Domainmodi extends Backend
{
    ...
    public function index(){
        if ($this->request->isAjax()){
            ...
            $aInfo = Db::name('admin')->field('id,nickname')->where('status','normal')->select();
            $aInfo = array_combine(array_column($aInfo, 'nickname'), array_column($aInfo, 'id'));
            if($admin){
            ...
            foreach($list as &$v){
                ...
                if($v['type']  == 8 && json_decode($v['ext'],true) ){
                    ...
                    $v['ext'] = "<a href='javascript:;' data-id={$v['id']} onclick='showDetail(this)' data-operate='ext' data-url='/admin/domain/domainmodi/show'>查看包内域名</a>";;
                }

                $v['show'] = "<a href='javascript:;' data-id={$v['id']} onclick='showDetail(this)' data-operate='old_value,new_value' data-url='/admin/domain/domainmodi/show'>详情</a>";
