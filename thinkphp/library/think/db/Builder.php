    ...
    protected function parseDateTime($value, $key, $options = [], $bindName = null, $bindType = null)
    {
        ...
        if (strpos($key, '.')) {
            ...
            if (isset($options['alias']) ) {
                //增加 json里面的别名搜索 2020/06/05
                $pos = array_search($table, $options['alias']);
                if(!$pos){
                   $pos = implode(',',array_filter(array_map(function($v) use ($table) { return  array_search($table, $v[0]); },$options['join'])));
                }
                $table = $pos ? $pos : $table;
            }
        } else {
