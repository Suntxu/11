...
!function ($) {
        ...
        for (var i in pColumns) {
            ...
            if (!vObjCol.checkbox && vObjCol.field !== 'operate' && vObjCol.searchable && vObjCol.operate !== false) {
                ...
                if (vObjCol.searchList) {
                    ...
                    if (/BETWEEN$/.test(vObjCol.operate) || /THOUSANDS$/.test(vObjCol.operate)) {
                        ...
                        htmlForm.push(sprintf('<div class="col-xs-6"><input type="%s" class="%s" name="%s" autocomplete="off" value="%s" placeholder="%s" id="%s" data-index="%s" %s %s></div>', type, addClass, vObjCol.field, defaultValueArr[0], placeholderArr[0], vObjCol.field, i, style, extend));
                        htmlForm.push(sprintf('<div class="col-xs-6"><input type="%s" class="%s" name="%s" autocomplete="off" value="%s" placeholder="%s" id="%s" data-index="%s" %s %s></div>', type, addClass, vObjCol.field, defaultValueArr[1], placeholderArr[1], vObjCol.field, i, style, extend));
                        htmlForm.push('</div>');
                    }else if(/TEXT$/.test(vObjCol.operate)) {
                        var placeholderArr = placeholder.toString().match(/\|/) ? placeholder.split('|') : [placeholder, placeholder];
                        htmlForm.push('<div class="row">');
                        htmlForm.push(sprintf('<div class="col-xs-12"><textarea type="%s" class="%s" autocomplete="off" name="%s" placeholder="%s,最多可搜索300行数据" id="%s" data-index="%s" %s %s style="height:10%;"  >%s</textarea></div>', type, addClass, vObjCol.field, placeholderArr[1], vObjCol.field, i, style, extend,defaultValue));
                        htmlForm.push('</div>');
                    }else if(/INT$/.test(vObjCol.operate) || /RANGE$/.test(vObjCol.operate)) {
                        htmlForm.push(sprintf('<input type="%s" class="%s" name="%s" value="%s" placeholder="%s" autocomplete="off" id="%s" data-index="%s" %s %s>', type, addClass, vObjCol.field, defaultValue, placeholder, vObjCol.field, i, style, extend));
    ...
    var getSearchQuery = function (that, removeempty) {
        ...
        $("form.form-commonsearch .operate", that.$commonsearch).each(function (i) {
            ...
            if (obj.size() > 1) {
                if (/BETWEEN$/.test(sym)|| /THOUSANDS$/.test(vObjCol.operate)) {
