define(['jquery', 'bootstrap', 'plupload', 'template'], function ($, undefined, Plupload, Template) {
    var Upload = {
        ...
        config: {
            ...
            //在这增加oss路径
            previewosstpl: '<li class="col-xs-3"><a href="'+Config.operate_url+'<%=fullurl%>" data-url="'+Config.operate_url+'<%=url%>" target="_blank" class="thumbnail"><img src="'+Config.operate_url+'<%=fullurl%>" class="img-responsive"></a></li>',
            previewossusertpl: '<li class="col-xs-3"><a href="'+Config.users_url+'<%=fullurl%>" data-url="'+Config.users_url+'<%=url%>" target="_blank" class="thumbnail"><img src="'+Config.users_url+'<%=fullurl%>" class="img-responsive"></a></li>',
            preterealtpl: '<li class="col-xs-3"><a href="'+Config.users_url+'alireal/<%=fullurl%>" data-url="'+Config.users_url+'alireal/<%=url%>" target="_blank" class="thumbnail"><img src="'+Config.users_url+'alireal/<%=fullurl%>" class="img-responsive"></a></li>',
            previewtpl: '<li class="col-xs-3"><a href="<%=fullurl%>" data-url="<%=url%>" target="_blank" class="thumbnail"><img src="<%=fullurl%>" class="img-responsive"></a></li>',
        },
            ...
            onUploadSuccess: function (up, ret) {
                ...
                if (button) {
                    ...
                    if (input_id) {
                        ...
                        urlArr.push(data.url+'?'+rand);
                        inputObj.val(urlArr.join(",")).trigger("change");
                        inputObj.val(data.url.replace(/\/uploads/,'')+'?'+rand);
                    ...
                    if (preview_id && input_id) {
                        $(document.body).on("keyup change", "#" + input_id, function () {
                            ...
                            $.each(inputArr, function (i, j) {
                                ...
                                //采用oss判断
                                var ef = $('#'+preview_id).data('oss');
                                if( ef == 'op'){
                                    var html = tpl ? Template(tpl, data) : Template.render(Upload.config.previewosstpl, data);
                                }else if(ef == 'user') {
                                    var html = tpl ? Template(tpl, data) : Template.render(Upload.config.previewossusertpl, data);               
                                }else if(ef == 'real'){
                                    var html = tpl ? Template(tpl, data) : Template.render(Upload.config.preterealtpl, data);
                                }else{
