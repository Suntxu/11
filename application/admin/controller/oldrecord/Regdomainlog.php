 ...
 * @icon fa fa-user
 */
class Regdomainlog extends Backend
{
    ...
    {
     	global $remodi_db;
        $this->model = Db::connect($remodi_db); //->table(PREFIX.'Task_record')
        parent::_initialize();
    }
    /**
     * 查看
    ...
    public function index($ids = '')
    {
        ...
        if ($this->request->isAjax()) {   
            ...
            list($where, $sort, $order, $offset, $limit,$group,$special_condition,$year,$nuid) = $this->buildparams();
            ...
            if($filter == '{}'){
                $total = $this->model->table(PREFIX.'Task_Detail'.$year)->where('TaskStatusCode',2)->count();
                // $total = $this->model->alias('r')->join(PREFIX.'Task_Detail d','r.id = d.taskid','left')
                //     ->where(['d.TaskStatusCode' => 2,'r.tasktype' => 2])
                //     ->count();
            }else{
                $total = $this->model->table(PREFIX.'Task_record')->alias('r')->join(PREFIX.'Task_Detail'.$year.' d','r.id = d.taskid','right')->join('domain_user u','r.userid=u.id','left')
                    ->where($where)->where(' d .TaskStatusCode = 2 and r.tasktype = 2 '.$def)
                    ->count();
            }
            $list = $this->model->table(PREFIX.'Task_record')->alias('r')->join(PREFIX.'Task_Detail'.$year.' d','r.id = d.taskid','right')->join('domain_user u','r.userid=u.id','left')
            ...
            $conm = 'SELECT sum(d.money) as n,sum(r.cos_price) as c FROM '.PREFIX.'Task_record as r right join '.PREFIX.'Task_Detail'.$year.' as d on r.id=d.taskid left join '.PREFIX.'domain_user as u on  r.userid= u.id '.$sql.' and r.tasktype = 2 AND d.TaskStatusCode=2 '.$def;
