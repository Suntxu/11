...
class Newtype extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = Db::name('domain_newstype');
    }
    ...
    public function index()
    {
        ...
        {   
            ...
            foreach($categorydata as $k => &$v){
                ...
                $categorydata[$k]['nr'] = Db::name('domain_news')->where('type1id='.$v['id'].' and zt != 99 ')->count();
            }
    ...
    public function edit($ids = NULL)
    {
        ...
        $data = $this->model->field('id,name1,xh,pid,zt')->where(['id'=>$ids])->find();
    ...
    public function del($ids='')
    {
       if($ids){
            ...
            if(empty($data['pid'])){
                ...
                //资讯删除
                Db::name('domain_news')->whereIn('type1id',$sid)->delete();
                //父级分类 关联删除
                $this->model->where('pid','=',$ids)->delete();
            }
            Db::name('domain_news')->where(['type1id'=>$ids])->delete();
