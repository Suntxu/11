    ...
    public function index()
    {
        if ($this->request->isAjax())
        {
            ...
            foreach($list as $k => $v){
               ...
               switch ($v['subtype']) {
                    case 0:
                        ...
                        if(strlen($v['infoid']) == 14 && substr($v['infoid'],0,1) == 'F'){
                          ...
                          $show = '/admin/vipmanage/recode/deallog?'.$u.$v['uid'].'&bc='.$v['infoid'];
                        }
               ...
               $list[$k]['subtype'] = Fun::ini()->getStatus($v['subtype'],[' 一口价域名交易',' push域名',' 域名注册','转回原注册商','用户充值','后台充值','发票申请','实名认证','现金提现','其他','怀米大使','域名续费','消保店铺保证金','域名预定','域名拼团','域名竞价','域名赎回','注册域名退款','域名回收','自动委托','域名手动续费','注册包购买','一口价退款']);
               $list[$k]['group'] = $v['uid'];
            }
