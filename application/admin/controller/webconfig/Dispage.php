...
class Dispage extends Backend
{
    ...
    public function index()
    {
        ...
        {   
            ...
            $list = $this->model->field('id,title,link,create_time,type,status,money')->where($where)->order($sort,$order)
                ->limit($offset, $limit)
                ->select();
    ...
    public function add()
    {
        if ($this->request->isPost()){
            ...
            // 如果没搜索的话 不能提交
            if(empty($params['title']) || empty($params['tdesc'] || empty($params['link']))){
                $this->error('请输入名称');
            }
