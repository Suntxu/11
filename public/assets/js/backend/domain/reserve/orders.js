    ...
    var Controller = {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        { field: 'r.pstatus', title: '交割状态',formatter: Table.api.formatter.status,searchList: {'0':'未支付','1':'未交割','2':'交割失败','3':'已交割','4':'违约'},notit:true, footerFormatter: function (data) {
                                var field = 'nopay';
                                var total_sum = data.reduce(function (sum, row) {
                                    return parseFloat(row[field]);
                                }, 0);
        ...
        uniquelist: function () {
            ...
            Table.api.init({
                showFooter: true,
                extend: {
                    index_url: 'domain/reserve/orders/uniquelist',
                    table: 'user',
                }
            });
            var table = $("#table");
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        { field: 'r.status', title: '订单状态',visible:false,searchList: {'0':'进行中','1':'已预定','2':'竞拍中','3':'预定失败','7':'得标','8':'未得标','10':'外部得标'},notit:true,},
                        { field: 'r.into', title: '订单类型',visible:false,searchList: {'0':'正常订单','1':'闯入订单'},notit:true},
                        { field: 'r.pstatus', title: '交割状态',visible:false,searchList: {'0':'未支付','1':'未交割','2':'交割失败','3':'已交割','4':'违约'},notit:true},
                        { field: 'r.time', title: '订单创建时间',addclass:'datetimerange',operate:'INT',formatter: Table.api.formatter.datetime,sortable:true },
                        { field: 'i.dtype', title: '域名类型',operate: 'RLIKE',searchList:$.getJSON('domain/manage/getDomainType'),
                            footerFormatter: function (data) {
                                var field = 'nopay';
