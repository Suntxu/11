...
class Ordersagentout extends Backend
{
    ...
    public function index()
    {
        if ($this->request->isAjax())
        {
            ...
            foreach($list as $k => $v){
                ...
                $list[$k]['pmoney'] = '<span style="color:orange;font-size:8px;">成本:'.$v['agent_cost'].'</span><br><span style="color:red;font-size:8px;">额外:'.($v['final_money'] - $v['agent_cost']).'</span>';
            }
    ...
    public function upmodi(){
        ...
        if($this->request->isAjax()){
            ...
            $flag = Db::name('domain_order_fx')->where($where)->whereIn('id',$id)->column('id');
            if(empty($flag)){
                $this->error('该记录不存在或状态已发生改变,请刷新页面！');
            }
            Db::name('domain_order_fx')->whereIn('id',$flag)->setField('rebate_type',$status);
