...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if($admin->status == 'hidden'){
            $this->setError('该账号已被禁止登录!');
            return false;
        }
        if (Config::get('fastadmin.login_failure_retry') && $admin->loginfailure >= 3 && time() - $admin->updatetime < 86400) { //
        ...
        if ($admin->password != md5(md5($password) . $admin->salt)) {
            ...
            $msg = '用户: '.$admin->nickname.'; IP: '.$request->ip().'; 请求时间: '.date('Y-m-d H:i:s')."\n";
            file_put_contents(APP_PATH.'/../public/logs/login_error.txt',$msg,FILE_APPEND);
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        //获取用户的归属组
        $a_id = \think\Db::name('auth_group_access')->field('group_id')->where(['uid'=>$user['id']])->find();
        // 读取管理员当前拥有的权限节点
        ...
        foreach ($ruleList as $k => &$v) {
            if($a_id['group_id'] == 2 || $a_id['group_id'] == 6){
                if($v['id'] == 85){
                    unset($ruleList[$k]);
                    continue;
                }
